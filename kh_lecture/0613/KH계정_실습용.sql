...
FROM EMPLOYEE;
SELECT * FROM EMPLOYEE; -- 별표는회사에서잘안씀
...
SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_NAME LIKE '%연'; -- 1번답
-- 연, 지연, 차지연, 내차지연, ...
...
(HIRE_DATE BETWEEN '90/01/01' AND '01/12/01') AND
SALARY >= 2700000;
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '___#_%' ESCAPE '#'; -- ESCAPE: 와일드카드기능을없애고 문자로 동작하게해준다.
